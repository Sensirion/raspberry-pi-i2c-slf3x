# Sensirion Raspberry Pi I²C SLF3X Driver

The repository provides a driver for setting up a sensor of the SLF3X family to run on a Raspberry Pi over I²C.
...
Use the following pins to connect your SLF3X:

| *SLF3X* | *Cable Color*  |   *Raspberry Pi*   |
| :----------------: | -------------- | ------------------ |
| SDA | green | Pin 3
| VDD | red | Pin 1
| GND | black | Pin 6
...
- Download the SLF3X driver from [Github](https://github.com/Sensirion/raspberry-pi-i2c-slf3x) and extract the `.zip` on your Raspberry Pi
- Connect the SLF3X sensor as explained in the [section above](#connect-the-sensor)
...
- Compile the driver
    ...
    2. Navigate to the driver directory. E.g. `cd ~/raspberry-pi-i2c-slf3x`
    3. Navigate to the subdirectory example-usage.
    4. Run the `make` command to compile the driver

       Output:
       ```
       rm -f slf3x_i2c_example_usage
       cc -Os -Wall -fstrict-aliasing -Wstrict-aliasing=1 -Wsign-conversion -fPIC -I. -o slf3x_i2c_example_usage  slf3x_i2c.h slf3x_i2c.c sensirion_i2c_hal.h sensirion_i2c.h sensirion_i2c.c \
           sensirion_i2c_hal.c sensirion_config.h sensirion_common.h sensirion_common.c slf3x_i2c_example_usage.c
       ```
- Test your connected sensor
    - Run `./slf3x_i2c_example_usage` in the same directory you used to
...
If you run `./slf3x_i2c_example_usage` but do not get sensor readings but something like this instead
...
-   Ensure that your user account has read and write access to the I²C device.
    If it only works with user root (`sudo ./slf3x_i2c_example_usage`), it's
